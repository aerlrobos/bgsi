...
do
    ...
    local OTCHub = v4.OTCHub

    if type(OTCHub) ~= "table" then
        OTCHub = {
			slots = {}
		}
        v4.OTCHub = OTCHub
    ...
    if type(OTCHub.slots) ~= "table" then
        OTCHub.slots = {}
    end
    ...
    do
        ...
        for k, v in pairs(OTCHub.slots) do
        ...
        if not v7 then
            v4.OTCCfgGen = (tonumber(v4.OTCCfgGen) or 0) + 1
        end
    ...
    if type(unload) == "function" then
        pcall(unload)
    elseif type(v4.OTCUnload) == "function" then
        local OTCUnloadUid = v4.OTCUnloadUid
        local v12 = OTCUnloadUid == nil or str == tostring(OTCUnloadUid)

        if OTCUnloadUid == nil then
            for k, v in pairs(OTCHub.slots) do
        ...
        if v12 then
            local OTCUnload = v4.OTCUnload

            if str == tostring(OTCUnloadUid or str) then
                v4.OTCUnload = nil
                v4.OTCUnloadUid = nil
            end
...
local t1 = {
	...
	Status = "preview",
	Game = "Steal an Egg",
	Discord = "https://discord.gg/TYV2cbVYYX",
	Website = "",
	Changelog = "",
	Author = "OTC (@aerlro)",
	Credits = "Thanks to @8wbh on Discord",
...
t1.LogoFile = "OTC" .. "/logo.png"
t1.LogoFileLight = "OTC" .. "/logo-light.png"
local n1 = 620
local n2 = 430
local n3 = 152
local Players = game:GetService("Players")
...
do
    ...
    do
        ...
        do
            ...
            function v51()
                ...
                local v352 = OTCHub.slots[str]

                if type(v352) ~= "table" then
                    v352 = {}
                    OTCHub.slots[str] = v352
                end

                return v352
            end

            t1.ConfigFile = (("OTC" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. v26(LocalPlayer and LocalPlayer.Name or "Player") .. "-config.json"
            t2 = {
				Dark = {
					...
					OTC = Color3.fromRGB(246, 242, 234)
				},
				Light = {
            ...
            t4 = {
				name = Enum.Font.BuilderSansBold,
            ...
            if PH_UI then
                local OTCUnloadUid = (getgenv and getgenv() or _G).OTCUnloadUid

                if OTCUnloadUid == nil or str == tostring(OTCUnloadUid) then
                    PH_UI:Destroy()
                end
            end
    ...
    local function v271(p58, p59)
        ...
        local v602 = "OTC" .. "/" .. v26(t1.Game)

        if type(makefolder) == "function" then
            pcall(makefolder, v602)
        end

        local v603 = ("OTC" .. "/" .. v26(t1.Game)) .. "/cache"

        if type(makefolder) == "function" then
            pcall(makefolder, v603)
        end

        local v604 = ("OTC" .. "/" .. v26(t1.Game)) .. "/configs"
    ...
    function v272(p60)
        ...
        if not v271((("OTC" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. v26(LocalPlayer and LocalPlayer.Name or "Player") .. "-config.json", result) then
            v271((("OTC" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. str .. "-config.json", result)
        end
    end
    ...
    local function v276()
        ...
        if type(listfiles) == "function" then
            local ok, result = pcall(listfiles, ("OTC" .. "/" .. v26(t1.Game)) .. "/configs")
    ...
    function v278(p68)
        ...
        local v647 = v271
        local v648 = ("OTC" .. "/" .. v26(t1.Game)) .. "/configs"
    ...
    function v279(p69, p70)
        ...
        local v654 = v275
        local v655 = ("OTC" .. "/" .. v26(t1.Game)) .. "/configs"
    ...
    function v280(p71)
        ...
        local v661, v662 = v275((("OTC" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. v26(LocalPlayer and LocalPlayer.Name or "Player") .. "-config.json")

        if not v661 then
            v661, v662 = v275((("OTC" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. str .. "-config.json")
        end

        if not v661 then
            local v663 = v275
            local v664 = ("OTC" .. "/" .. v26(t1.Game)) .. "/configs"
        ...
        if not v661 then
            local t85 = {
				("OTC" .. "/" .. v26(t1.Game)) .. "/config.json",
				"OTC" .. "/config.json",
				"OTC" .. "/" .. t1.Game .. "/config.json",
				("OTC" .. "/" .. v26(t1.Game)) .. "/OTC.json",
				"OTC" .. "/OTC.json",
				"OTC.json"
			}
        ...
        if v662 ~= (("OTC" .. "/" .. v26(t1.Game)) .. "/cache") .. "/" .. v26(LocalPlayer and LocalPlayer.Name or "Player") .. "-config.json" then
            v272(true)
        end
...
local v294 = v259("About", "OTC for Steal an Egg")
...
v289(v300, "SaveCfgAs", "Save config", "writes OTC/.../configs/<name>.json", "Save", function()
    v278(t9.CfgSaveName)
end, true)
...
-- ============================================================================
-- Rayfield Gen2 UI bridge
-- UI replacement only: the existing OTC UI bridge keeps the original logic, t9 config and t10 callbacks
-- remain the source of truth. No feature logic is reimplemented here.
-- ============================================================================
local OTCRayfieldOk, OTCRayfield = pcall(function()
    return loadstring(game:HttpGet("https://sirius.menu/gen2"))()
end)

if OTCRayfieldOk and OTCRayfield then
    local OTCWindow = OTCRayfield:CreateWindow({
        name = "OTC・Steal an Egg",
        subtitle = "by aerlro",
        theme = "frost",
    ...
    local function otcOn(name, value)
        t9[name] = value
        local widget = t10[name]

        if widget and widget.on then
            pcall(widget.on, value)
        end
    ...
    local function otcToggle(tab, name, description)
        tab:CreateToggle({
            name = name,
            ...
            callback = function(value)
                otcOn(name, value == true)
            end,
        })
    end

    local function otcSlider(tab, name, minimum, maximum, increment, suffix, description)
        tab:CreateSlider({
            name = name,
            description = description,
            ...
            increment = increment,
            suffix = suffix or "",
            value = tonumber(t9[name]) or minimum,
            flag = "OTC_" .. name,
            callback = function(value)
                otcOn(name, value)
            end,
    ...
    local function otcDropdown(tab, name, options, multiple, description)
        local current = t9[name]
        local currentOption

        if multiple then
            currentOption = type(current) == "table" and current or {}
        else
            currentOption = { tostring(current or options[1] or "") }
        end
        ...
        tab:CreateDropdown({
            ...
            callback = function(value)
                if multiple then
                    otcOn(name, type(value) == "table" and value or {})
                else
                    otcOn(name, type(value) == "table" and value[1] or value)
                end
            end,
        })
    end

    local function otcInput(tab, name, placeholder, description)
        tab:CreateInput({
            name = name,
            description = description,
            value = tostring(t9[name] or ""),
    ...
    local function otcButton(tab, name, callback, description)
        tab:CreateButton({
            name = name,
            description = description,
            callback = callback,
    ...
    local function otcKeybind(tab, name, current, description)
        local currentKey = current or Enum.KeyCode.RightShift
        ...
        tab:CreateKeybind({
            ...
            flag = "OTC_" .. name,
            callback = function()
                -- The original script already owns the actual keybind action.
                -- Rayfield only provides the keybind editor here.
            end,
            onChanged = function(value)
                if name == "OpenBind" then
                    t9.OpenBind = value
                    pcall(function()
                        OTCWindow.settings.toggleKeybind = value
                    end)
                elseif name == "FlightBind2" then
                    t9.FlightBind = value
                    if t10.FlightBind2 and t10.FlightBind2.on then
                        pcall(t10.FlightBind2.on, value)
                    end
                elseif name == "FlightBind" then
                    t9.FlightBind = value
                    if t10.FlightBind2 and t10.FlightBind2.on then
                        pcall(t10.FlightBind2.on, value)
                    end
    ...
    -- About
    -- ------------------------------------------------------------------------
    local About = OTCWindow:CreateTab({ name = "About", icon = "info" })
    About:CreateText({
        name = t1.Title or "OTC",
        text = string.format(
            "v%s\n%s\n%s",
            tostring(t1.Version or "?"),
            tostring(t1.Game or "Steal an Egg"),
            tostring(t1.Tagline or "Autofarm, hatch eggs and other stuff")
        ),
    })
    About:CreateSection("Features")
    About:CreateText({ name = "Auto Steal", text = "Snatch eggs and bring them to safe zone" })
    About:CreateText({ name = "Plot", text = "Place, hatch eggs, auto sell" })
    About:CreateText({ name = "Serverhop", text = "Idle, time, or night spawn — Auto hop only leaves if one of those boxes has a number" })
    About:CreateText({ name = "Misc", text = "ESP, stats, index claim, anti trap / ragdoll, bat aura, flight, bypass speed, optimizer" })
    About:CreateText({ name = "Webhook", text = "Discord embeds with the pet/egg icon on steal, hatch, and sell" })

    if type(t1.Discord) == "string" and t1.Discord ~= "" then
        otcButton(About, "Copy Discord", function()
            if setclipboard then pcall(setclipboard, t1.Discord) end
        end)
    end
    if type(t1.Website) == "string" and t1.Website ~= "" then
        otcButton(About, "Copy Website", function()
            if setclipboard then pcall(setclipboard, t1.Website) end
        end)
    end
    if type(t1.Changelog) == "string" and t1.Changelog ~= "" then
        About:CreateText({ name = "What's new", text = t1.Changelog })
    end
    About:CreateSection("Credits")
    About:CreateText({ name = "Made by", text = tostring(t1.Author or t1.Title or "OTC") })
    if t1.Credits then
        About:CreateText({ name = "With", text = tostring(t1.Credits) })
    end
    if t1.Support then
        About:CreateText({ name = "Support", text = tostring(t1.Support) })
    end
    About:CreateSection("Disclaimer")
    About:CreateText({
        name = "Not official",
        text = "Not affiliated with " .. tostring(t1.Game or "this game") .. ". I'm not responsible for any bans, use at your own risk !",
    })

    -- ------------------------------------------------------------------------
    -- Auto Steal
    -- ------------------------------------------------------------------------
    local AutoStealTab = OTCWindow:CreateTab({ name = "Auto Steal", icon = "crosshair" })
    AutoStealTab:CreateSection("Auto steal")
    otcToggle(AutoStealTab, "AutoSteal", "Snatch eggs and bring them to safe zone")
    otcDropdown(AutoStealTab, "StealTravel", { "Speed", "Flight" }, false, "Travel mode used by Auto Steal")
    otcSlider(AutoStealTab, "StealSpeed", 50, 1300, 1, " studs/s", "Studs/s — can pullback if this is too high")

    AutoStealTab:CreateSection("Targeting")
    otcDropdown(AutoStealTab, "StealMode", { "Best value", "Egg type filter", "Gen ($/s) snipe" }, false, "Filters being used from below")
    otcInput(AutoStealTab, "GenSnipeFloor", "any · e.g. 100m", "What the pet inside of the egg will pay")
    otcDropdown(AutoStealTab, "Areas", t5, true, "Which zones to steal from")

    AutoStealTab:CreateSection("What qualifies")
    otcToggle(AutoStealTab, "UseRarity", "Off = any egg type counts")
    otcDropdown(AutoStealTab, "Rarities", t6, true, "An egg counts if it is one of these")
    otcToggle(AutoStealTab, "UseMutation", "Off = mutated or not, both fine")
    otcDropdown(AutoStealTab, "Mutations", t7, true, "An egg counts if it carries one of these")
    otcInput(AutoStealTab, "MinWeight", "any", "Minimum weight (Kg) — blank for any")

    AutoStealTab:CreateSection("Event")
    otcToggle(AutoStealTab, "AutoEvent", "After filters: grab infested, equip, feed")
    otcInput(AutoStealTab, "EventKeepGen", "feed any · e.g. 5m", "Keeps high-pay eggs — blank = feed any")

    -- ------------------------------------------------------------------------
    -- Plot
    -- ------------------------------------------------------------------------
    local PlotTab = OTCWindow:CreateTab({ name = "Plot", icon = "home" })
    PlotTab:CreateSection("Eggs & pets")
    otcToggle(PlotTab, "AutoPlaceEggs", "Idle · placed 0 · hatched 0")
    otcDropdown(PlotTab, "NeverPlaceRarity", { "Place all", unpack(t6) }, false, "Keeps better eggs in inventory")
    otcInput(PlotTab, "PlaceMinGen", "any · e.g. 1.5m", "Only place eggs worth ($/s)")
    otcToggle(PlotTab, "AutoHatch", "Hatches every egg the moment its timer is up")
    otcToggle(PlotTab, "EquipBest", "Uses the game's own equip-best")

    PlotTab:CreateSection("Upgrades")
    otcToggle(PlotTab, "UpgTrails", "Idle · bought 0 · sold 0")
    otcToggle(PlotTab, "UpgTreadmill", "Buys the next treadmill when you can afford it")
    otcToggle(PlotTab, "UpgPen", "More room for pets")
    otcInput(PlotTab, "KeepMoney", "spend it all · e.g. 500m", "Never spend below this — blank to spend freely")

    PlotTab:CreateSection("Selling")
    otcButton(PlotTab, "Preview what will sell", function()
        local widget = t10.SellPreview
        if widget and widget.on then pcall(widget.on) end
    end, "Opens a window under the stats panel")
    otcInput(PlotTab, "SellUnderGen", "nothing sells · e.g. 250k", "Blank = nothing sells")
    otcToggle(PlotTab, "AutoSellPets", "Equips then sells at the stall · worst first · only below the floor")
    otcToggle(PlotTab, "AutoSellEggs", "Equips then sells at the stall · spare eggs only — plot eggs stay")

    PlotTab:CreateSection("Treadmill training")
    otcToggle(PlotTab, "AutoTreadmill", "Not training · 0/s · earned 0 this session")
    otcToggle(PlotTab, "TrainWhenIdle", "Only when Auto Steal is off. Auto Steal never wears the belt")
    otcSlider(PlotTab, "ReadyEarly", 0, 15, 1, "s before reset", "Step earlier to steal")

    -- ------------------------------------------------------------------------
    -- Serverhop
    -- ------------------------------------------------------------------------
    local HopTab = OTCWindow:CreateTab({ name = "Serverhop", icon = "server" })
    HopTab:CreateSection("Auto hop")
    otcToggle(HopTab, "AutoHop", "Off · 0 hops this session")
    HopTab:CreateSection("Leave when")
    otcInput(HopTab, "HopIdle", "off · 90", "Auto Steal on, nothing banked. Night does not count. Min 5. Blank = off.")
    otcInput(HopTab, "HopAfter", "off · 20", "Leave after this long no matter what. Min 1. Blank = off.")
    HopTab:CreateSection("Leave now")
    otcButton(HopTab, "Hop now", function()
        local widget = t10.HopNow
        if widget and widget.on then pcall(widget.on) end
    end, "One hop. Auto hop can stay off.")
    HopTab:CreateSection("Which servers")
    otcSlider(HopTab, "HopPages", 1, 10, 1, " pages", "3 is enough. more pages is slower.")
    otcToggle(HopTab, "HopSkipFull", "A full server just dumps you back here")
    otcDropdown(HopTab, "HopPlayers", { "Lowest", "Highest" }, false, "Lowest = emptier, Highest = fuller")

    -- ------------------------------------------------------------------------
    -- Misc
    -- ------------------------------------------------------------------------
    local MiscTab = OTCWindow:CreateTab({ name = "Misc", icon = "settings-2" })
    MiscTab:CreateSection("Eggs on the map")
    otcToggle(MiscTab, "EggESP", "Show ESP for eggs")
    otcDropdown(MiscTab, "ESPFilter", { "All eggs", "Eggs matching my filters", "Stolen target only" }, false)
    otcToggle(MiscTab, "ESPBeam", "Line to the egg Auto Steal picked")
    MiscTab:CreateSection("Eggs on your plot")
    otcToggle(MiscTab, "PlotESP", "Payout and hatch timer — green when ready")
    MiscTab:CreateSection("Stats")
    otcToggle(MiscTab, "StatsPanel", "Draggable: money, income, pen, best pet, best egg, speed, session")
    otcToggle(MiscTab, "ClaimIndex", "Redeems every completed index entry in one sweep")
    MiscTab:CreateSection("Defence")
    otcToggle(MiscTab, "AntiTrap", "Disables traps")
    otcToggle(MiscTab, "AntiMob", "Guards and other players' bats cannot flop or knock you")
    MiscTab:CreateSection("Bat")
    otcToggle(MiscTab, "BatAura", "Swings at any player in range")
    MiscTab:CreateSection("Walking")
    otcToggle(MiscTab, "BypassSpeed", "Off = normal walk. On = bypass cap")
    otcSlider(MiscTab, "BypassCap", 150, 1300, 1, " studs/s", "Bypass speed cap")
    MiscTab:CreateSection("Flight")
    otcToggle(MiscTab, "Flight", "WASD to fly, Space up, Left Ctrl down")
    otcSlider(MiscTab, "FlightSpeed", 150, 1300, 1, " studs/s", "Flight speed")
    otcKeybind(MiscTab, "FlightBind", t9.FlightBind or t1.FlightBind, "Toggle flight without opening the window")
    MiscTab:CreateSection("Performance")
    otcToggle(MiscTab, "Optimizer", "Lowest gfx: shadows, particles, lights, post-fx, terrain water")
    otcSlider(MiscTab, "FPSCap", 0, 240, 1, "", "0 = no fps cap")

    -- ------------------------------------------------------------------------
    -- Webhook
    -- ------------------------------------------------------------------------
    local WebhookTab = OTCWindow:CreateTab({ name = "Webhook", icon = "webhook" })
    WebhookTab:CreateSection("Connection")
    otcToggle(WebhookTab, "HookEnabled", "Send outbound")
    otcInput(WebhookTab, "HookUrl", "https://", "Paste a URL — never committed")
    otcButton(WebhookTab, "Test send", function()
        local widget = t10.HookTest
        if widget and widget.on then pcall(widget.on) end
    end, "Posts a sample embed to the URL above")
    WebhookTab:CreateSection("What to send")
    otcToggle(WebhookTab, "HookStolen", "Icon, $/s, rarity, mutations, where it came from")
    otcToggle(WebhookTab, "HookHatched", "Icon, $/s, rarity, weight")
    otcToggle(WebhookTab, "HookSold", "Icon and what it was earning")
    otcToggle(WebhookTab, "HookRewards", "Index redeem count")
    WebhookTab:CreateSection("How much noise")
    otcInput(WebhookTab, "HookMinGen", "everything · e.g. 50m", "Stolen and hatched — blank for everything")
    otcDropdown(WebhookTab, "HookRarityFloor", { "Any", unpack(t6) }, false, "Stolen and hatched below this rarity are skipped")
    otcToggle(WebhookTab, "SessionDigest", "Every 10 minutes — stolen, lost, hatched, sold, cash")
    WebhookTab:CreateSection("The message")
    otcDropdown(WebhookTab, "HookPing", { "No ping", "Here", "User id" }, false)
    otcInput(WebhookTab, "HookUserId", "0", "Optional")
    otcToggle(WebhookTab, "HookUsername", "Show my Roblox name and headshot")
    otcToggle(WebhookTab, "ExportUrl", "Let exported configs carry the URL")

    -- ------------------------------------------------------------------------
    -- Settings
    -- ------------------------------------------------------------------------
    local SettingsTab = OTCWindow:CreateTab({ name = "Settings", icon = "sliders-horizontal" })
    SettingsTab:CreateSection("Appearance")
    otcToggle(SettingsTab, "PhoneUI", "Compact hub for phones / emulators")
    otcSlider(SettingsTab, "UIScale", 75, 125, 1, "%", "UI scale")
    otcDropdown(SettingsTab, "Theme", { "Dark", "Light" }, false, "Dark or light")

    SettingsTab:CreateSection("Keybinds")
    otcKeybind(SettingsTab, "OpenBind", t9.OpenBind or t1.OpenBind, "Open / close the UI")
    otcKeybind(SettingsTab, "FlightBind2", t9.FlightBind or t1.FlightBind, "Same bind as the movement page")
    otcToggle(SettingsTab, "StartMin", "Start minimised")

    SettingsTab:CreateSection("Config")
    otcDropdown(SettingsTab, "CfgPreset", { "default" }, false, "Load config")
    otcInput(SettingsTab, "CfgSaveName", "name · or leave blank", "Save as")
    otcButton(SettingsTab, "Save config", function()
        v278(t9.CfgSaveName)
    end, "Writes the existing OTC config")
    otcButton(SettingsTab, "Export settings", function()
        v272()
        local ok, result = pcall(function()
            return HttpService:JSONEncode((v270()))
        end)
    ...
    end, "Copies the existing config to clipboard")
    otcInput(SettingsTab, "ImportPaste", "paste, then press Import", "Load config from clipboard")
    otcButton(SettingsTab, "Import", function()
        local ImportPaste = t9.ImportPaste
    ...
    otcButton(SettingsTab, "Reset position & size", function()
        v113.Position = UDim2.fromScale(0.5, 0.5)
        v113.Size = UDim2.fromOffset(n1, n2)
        v108.Scale = 1
        ...
        u305()
    end, "Window and orb back to the middle at default size")

    -- The original UI remains alive because the feature engine still owns t10,
    -- but it is made invisible. Rayfield is now the only visible UI layer.
    pcall(function()
        v98.Enabled = false
    end)
end
...
v325.UI = t149
v324.OTCUI = type(v324.OTCUI) == "table" and v324.OTCUI or {}
v324.OTCUI[str] = t149
if type(v324.UI) ~= "table" or not (function()
    local OTCHub = (getgenv and getgenv() or _G).OTCHub
    local v354 = type(OTCHub) == "table" and OTCHub.slots
...
(function()
    ...
    local function v1144()
        ...
        if v1494 then
            for _, child in ipairs(workspace:GetChildren()) do
                if child.Name == "OTCSupport" or child.Name == "Hub45Support" then
                    pcall(function()
                        child:Destroy()
                    end)
                end
    ...
    local u1195 = (function()
        ...
        local function v1810(p269, p270, p271)
            ...
            if p271 and p271.bytes then
                local v2996 = "OTC" .. tostring(math.floor(os.clock() * 1000000)) .. tostring(math.random(100000, 999999))
        ...
        local function v1811(p272)
            ...
            local str9 = tostring(p272.title or "OTC")
            local v3014
            local g3028
            local str10
            local s17
            ...
            t184.text = v3018 == "" and "OTC" or "OTC · " .. v3018
            t183.footer = t184
    ...
    u1134 = (function()
        ...
        local function v2119()
            local OTCEsp = workspace:FindFirstChild("OTCEsp")

            if OTCEsp and OTCEsp:IsA("Folder") then
                t218.folder = OTCEsp
        ...
        local function v2120()
            ...
            if PlayerGui then
                local v3251 = PlayerGui:FindFirstChild(v49) or PlayerGui:FindFirstChild("OTCWorldGui")
        ...
        local function v2143(p370)
            if p370 then
                ...
                elseif t218.world and p370:IsDescendantOf(t218.world) then
                    ...
                    v3530 = p370Name == "OTCSupport" or (p370Name == "Hub45Support" or (p370Name == "OTCWorldGui" or (p370Name == v49 or p370Name == v48)))
                end
    ...
    local u1230 = (function()
        ...
        v2192.OTCHopUsed = type(v2192.OTCHopUsed) == "table" and v2192.OTCHopUsed or {}
        v2192.OTCHopRing = type(v2192.OTCHopRing) == "table" and v2192.OTCHopRing or {}
        v2192.OTCHopCache = type(v2192.OTCHopCache) == "table" and v2192.OTCHopCache or {}
        if type(v2192.OTCHopUntil) == "number" then
            ...
            if math.abs(v2195 - v2192.OTCHopUntil) < 10 then
            end
        end
        ...
        local function v2197(p390)
            ...
            local OTCHopRing = v2192.OTCHopRing

            for i = #OTCHopRing, 1, -1 do
                if p390 == OTCHopRing[i] then
                    table.remove(OTCHopRing, i)
                end
            end

            OTCHopRing[#OTCHopRing + 1] = p390

            while #OTCHopRing > 48 do
                table.remove(OTCHopRing, 1)
            end
        end
        local function v2198(p391)
            ...
            local g3859
            local v3858
            for i = 1, #OTCHopRing do
                if str28 == OTCHopRing[i] then
                    v3858 = true
                    g3859 = true
                end
            ...
            local v3860 = v2192.OTCHopUsed[str28]
            if type(v3860) ~= "number" then
                return false
            end
            if os.time() - v3860 > 2100 then
                v2192.OTCHopUsed[str28] = nil
        ...
        local function v2199()
            ...
            local ok54, result54 = pcall(readfile, (("OTC" .. "/" .. v26(t1.Game)) .. "/cache") .. "/hop-used.json")
            ...
            if type(data.jobs) == "table" then
                for k, v in pairs(data.jobs) do
                    ...
                    if v3867 and timestamp - v3867 <= 2100 and v3867 > (tonumber(v2192.OTCHopUsed[str29]) or 0) then
                        v2192.OTCHopUsed[str29] = v3867
                    end
                end
            end
            if type(data.ring) == "table" then
                ...
                for i = 1, #v2192.OTCHopRing do
                    local v3874 = v2192.OTCHopRing[i]
                ...
                v2192.OTCHopRing = t282

                while #v2192.OTCHopRing > 48 do
                    table.remove(v2192.OTCHopRing, 1)
                end
            end
        end
        local function v2200()
            ...
            local v3875 = "OTC" .. "/" .. v26(t1.Game)

            if type(makefolder) == "function" then
                pcall(makefolder, v3875)
            end

            local v3876 = ("OTC" .. "/" .. v26(t1.Game)) .. "/cache"

            if type(makefolder) == "function" then
                pcall(makefolder, v3876)
            end

            local v3877 = ("OTC" .. "/" .. v26(t1.Game)) .. "/configs"
            ...
            for k, v in pairs(v2192.OTCHopUsed) do
            ...
            if ok55 and type(result55) == "string" then
                pcall(writefile, (("OTC" .. "/" .. v26(t1.Game)) .. "/cache") .. "/hop-used.json", result55)
            end
        end
        v2199()
        local str30 = tostring(tostring(game.JobId) or "")
        if str30 ~= "" then
            v2192.OTCHopUsed[str30] = os.time()
            v2197(str30)
        end
        v2200()
        ...
        local function v2205(p395)
            local OTCHopCache = v2192.OTCHopCache

            if type(OTCHopCache) ~= "table" or (OTCHopCache.key ~= v2203() or type(OTCHopCache.list) ~= "table" or #OTCHopCache.list == 0) then
                OTCHopCache = nil

                if type(readfile) == "function" then
                    local ok58, result58 = pcall(readfile, (("OTC" .. "/" .. v26(t1.Game)) .. "/cache") .. "/hop-list.json")
                    ...
                    if ok58 and type(result58) == "string" and result58 ~= "" then
                        ...
                        if type(data) == "table" and data.key == v2203() and type(data.list) == "table" and #data.list > 0 then
                            OTCHopCache = data
                            v2192.OTCHopCache = data
                        end
                    end
                end
            end

            if type(OTCHopCache) ~= "table" or type(OTCHopCache.list) ~= "table" or #OTCHopCache.list == 0 then
                return
            end

            local v3913 = os.time() - (tonumber(OTCHopCache.at) or 0)

            if v3913 > 90 and not p395 then
                return
            end

            v2204(OTCHopCache.list)
        ...
        local function v2206(p396)
            ...
            if type(writefile) == "function" then
                ...
                local v3916 = "OTC" .. "/" .. v26(t1.Game)

                if type(makefolder) == "function" then
                    pcall(makefolder, v3916)
                end

                local v3917 = ("OTC" .. "/" .. v26(t1.Game)) .. "/cache"

                if type(makefolder) == "function" then
                    pcall(makefolder, v3917)
                end

                local v3918 = ("OTC" .. "/" .. v26(t1.Game)) .. "/configs"
                ...
                if ok59 and type(result59) == "string" then
                    pcall(writefile, (("OTC" .. "/" .. v26(t1.Game)) .. "/cache") .. "/hop-list.json", result59)
                end
            end
        end
        ...
        local function v2208()
            ...
            for i = math.max(1, #OTCHopRing - 7), #OTCHopRing do
                local v3939 = OTCHopRing[i]
        ...
        local function v2209()
            ...
            if str35 ~= "" then
                v2192.OTCHopUsed[str35] = os.time()
                v2197(str35)
            end
        ...
        local function v2211(p399)
            ...
            while u1117 and u2179 do
                ...
                v3984.OTCHopUntil = (not ok62 or type(result62) ~= "number") and 0 or math.max(0, result62)
                u2178 = true
    ...
    function u323()
        ...
        local ok103, result99, _, _ = pcall(function()
            ...
            if PlayerGui then
                local OTCWorldGui = PlayerGui:FindFirstChild("OTCWorldGui")
        ...
        if type(v2668.OTCUI) == "table" then
            v2668.OTCUI[str] = nil
        end

        if type(v2668.OTCDumpByUser) == "table" then
            v2668.OTCDumpByUser[str] = nil
        end
        ...
        if v2668.OTCUnloadUid == str then
            v2668.OTCUnloadUid = nil
            v2668.OTCUnload = nil
        end
    ...
    v1244.unload = u323
    v1244.alive = true
    v1243.OTCUnloadUid = str
    function v1243.OTCUnload()
        local LocalPlayer2 = Players.LocalPlayer
        local str42 = tostring(LocalPlayer2 and LocalPlayer2.UserId or 0)
        local OTCHub = v1243.OTCHub
        local v2673 = type(OTCHub) == "table" and (type(OTCHub.slots) == "table" and OTCHub.slots[str42])

        if type(v2673) == "table" and type(v2673.unload) == "function" then
            v2673.unload()
        end
    end
    function t149.OTCDump()
    ...
    local v1245 = v50()
    v51().dump = t149.OTCDump
    v1245.OTCDumpByUser = type(v1245.OTCDumpByUser) == "table" and v1245.OTCDumpByUser or {}
    v1245.OTCDumpByUser[str] = t149.OTCDump
    function v1245.OTCDump(...)
        local LocalPlayer3 = Players.LocalPlayer
        local str43 = tostring(LocalPlayer3 and LocalPlayer3.UserId or 0)
        local v2683 = v1245.OTCDumpByUser and v1245.OTCDumpByUser[str43]
