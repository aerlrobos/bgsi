...
for id, data in pairs(RiftsModule) do
    if data.Type == "Egg" and not data.Ignore then
...
for potionName, potionInfo in pairs(PotionsModule) do
    if potionInfo.OneLevel then
        table.insert(PotionList, potionName.." 1")
        PotionMap[potionName.." 1"] = {Name=potionName, Level=1}
    else
        local maxLevel = 0
        if potionInfo.CraftingCosts then
            maxLevel = #potionInfo.CraftingCosts
        elseif potionInfo.Buff and potionInfo.Buff.Expiry then
            maxLevel = #potionInfo.Buff.Expiry
        end
        maxLevel = math.max(maxLevel,1)
        for lvl = 1, maxLevel do
            local fullName = potionName.." "..lvl
            table.insert(PotionList, fullName)
            PotionMap[fullName] = {Name=potionName, Level=lvl}
        end
    end
end
...
local function createDropdown(section, title, callback)
    return section:AddDropdown(title, {Title=title, Values=PotionList, Multi=false, Default=nil, Callback=callback})
end
local function createToggle(section, title, callback)
    return section:AddToggle(title, {Title=title, Default=false, Callback=callback})
end

-- Use Potions Section
local UseSection = Tabs.Potions:CreateSection("▶️ Use Potions")
createDropdown(UseSection,"Potions",function(val)
    local d = PotionMap[val]
    if d then
        getgenv().PotionsConfig.SelectedUsePotion = d.Name
        getgenv().PotionsConfig.SelectedUsePotionLevel = d.Level
    end
end)
createToggle(UseSection,"Use Potions",function(val)
    getgenv().PotionsConfig.UsePotions = val
end)

-- Auto Use Loop
task.spawn(function()
    while true do
        if getgenv().PotionsConfig.UsePotions and getgenv().PotionsConfig.SelectedUsePotion then
            pcall(function()
                RemoteEvent:FireServer("UsePotion", getgenv().PotionsConfig.SelectedUsePotion, getgenv().PotionsConfig.SelectedUsePotionLevel)
            end)
        end
        task.wait(0.5)
    end
...
-- Craft Potions Section
local CraftSection = Tabs.Potions:CreateSection("▶️ Craft Potions")
createDropdown(CraftSection,"Potions",function(val)
    local d = PotionMap[val]
    if d then
        getgenv().PotionsConfig.SelectedCraftPotion = d.Name
        getgenv().PotionsConfig.SelectedCraftPotionLevel = d.Level
    end
end)
createToggle(CraftSection,"Craft Potions",function(val)
    getgenv().PotionsConfig.CraftPotions = val
end)

-- Auto Craft Loop
task.spawn(function()
    while true do
        if getgenv().PotionsConfig.CraftPotions and getgenv().PotionsConfig.SelectedCraftPotion then
            pcall(function()
                RemoteEvent:FireServer("CraftPotion", getgenv().PotionsConfig.SelectedCraftPotion, getgenv().PotionsConfig.SelectedCraftPotionLevel, false)
            end)
        end
        task.wait(0.5)
    end
...
local potionList={}
for _,p in ipairs(ShrineValues)do local n=p.Name if p.Type=="Potion" and p.Level then n=n.." "..p.Level end table.insert(potionList,n) end
table.sort(potionList)
ShrineSection:AddDropdown("SelectPotion",{Title="Potions",Values=potionList,Multi=false,Default=nil,Callback=function(v)getgenv().MiscConfig.SelectedPotion=v end})
ShrineSection:AddInput("PotionQuantity",{Title="Quantity",Default="1",Numeric=true,Finished=true,Callback=function(v)getgenv().MiscConfig.PotionQuantity=tonumber(v)or 1 end})
ShrineSection:AddToggle("AutoDonateShrine",{Title="Auto Donate Shrine",Default=false,Callback=function(v)getgenv().MiscConfig.AutoDonateShrine=v end})

local function getPotionByName(name)for _,p in ipairs(ShrineValues)do local n=p.Name if p.Type=="Potion" and p.Level then n=n.." "..p.Level end if n==name then return p end end end
...
task.spawn(function()
    while true do
        ...
        if cfg.AutoDonateShrine and cfg.SelectedPotion then
            ...
            if potion then
                local qty = math.clamp(tonumber(cfg.PotionQuantity) or 1, 1, 1000)
                pcall(function()
...
Tabs.Misc:CreateToggle("AutoClaimPrizes", {
    Title = "Auto Claim Prizes",
    Default = false,
    Callback = function(state)
        getgenv().AutoClaimPrizes = state
        if not state then return end

        task.spawn(function()
            local RS = game:GetService("ReplicatedStorage")
            local PrizeData = require(RS.Shared.Data.Prizes)
            local playerData = LocalData:Get()

            -- Sortăm cheile premiilor
            local keys = {}
            for key, data in pairs(PrizeData) do
                if typeof(key) == "string" and data.Requirement then
                    table.insert(keys, key)
                end
            end
            table.sort(keys)

            -- Începem auto-claim loop
            while getgenv().AutoClaimPrizes do
                pcall(function()
                    for _, key in ipairs(keys) do
                        if not (playerData.Prizes and playerData.Prizes[key]) then
                            RemoteEvent:FireServer("ClaimPrize", key)
                            task.wait(0.25)
                        end
                    end
                end)
